...
from loader.llff import load_llff_data
from loader.blender import load_blender_data
from utils import to8b, mse2psnr, img2mse
from utils.sampling import sample_pdf
from utils.embedder import get_embedder
from utils.ray import get_rays, get_rays_np, ndc_rays
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    weights = (
        alpha
        * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), 1.0 - alpha + 1e-10], -1), -1)[
            :, :-1
        ]
    )
        ...
        if use_batching:
            ...
            if N_rand is not None:
                ...
                if i < args.precrop_iters:
                    ...
                    if i == start:
                        print(
                            f"[Config] Center cropping of size {2*dH} x {2*dW} "
